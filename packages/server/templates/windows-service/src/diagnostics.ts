...
async function runDiagnostics() {
  ...
  const pollInterval = process.env.POLL_INTERVAL?.trim();
  ...
  if (!apiKey) {
    ...
    log({
      test: 'API_KEY',
      status: 'PASS',
      message: `Set (length: ${apiKey.length} chars) - starts with: ${apiKey.substring(0, 10)}...`
    });
  }
  ...
  try {
    ...
    if (error.response?.status === 401 || error.response?.status === 403) {
      log({
        ...
        details: `Status: ${error.response?.status}, Response: ${JSON.stringify(error.response?.data)}, API Key length: ${apiKey?.length}, First 10 chars: ${apiKey?.substring(0, 10)}`
      });
    } else {
      log({
        test: 'API Key Valid',
